...
/** `WordCount` experiment fixtures for the 'kmeans' bundle. */
@Configuration
class wordcount extends ApplicationContextAware {
  ...
  def `datagen.words`: FlinkJob = new FlinkJob(
    ...
    command =
      """
        |-v -c eu.e2data.benchmarks.kmeans.datagen.flink.WordGenerator        \
        |${app.path.datagens}/kmeans-datagens-1.0-SNAPSHOT.jar        \
  ...
  def `wordcount.default`: ExperimentSuite = {
    val `wordcount.flink.default` = new FlinkExperiment(
      ...
      command =
        """
          |-v -c eu.e2data.benchmarks.kmeans.flink.FlinkWC                      \
          |${app.path.apps}/kmeans-flink-jobs-1.0-SNAPSHOT.jar          \
          |${system.hadoop-2.path.input}/rubbish.txt                            \
